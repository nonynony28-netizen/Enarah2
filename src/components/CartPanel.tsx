  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={`relative z-10 w-[90%] sm:w-full sm:max-w-md h-full bg-white shadow-2xl flex flex-col justify-between border-slate-200 transition-transform duration-300 ease-out text-slate-900 ${
          isAr 
      ...
      >
        <div className="relative z-10 flex flex-col h-full justify-between">
          ...
          {/* Header */}
          <div className="flex flex-col border-b border-slate-200 bg-slate-50 px-6 py-5">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                <span className="px-2.5 py-1 rounded-md bg-blue-50 border border-blue-200 text-blue-700 text-xs font-bold">
                  {cartCount} {isAr ? 'قطع' : 'items'}
                </span>
                <button
                  onClick={() => setIsCartOpen(false)}
                  className="w-9 h-9 flex items-center justify-center rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-600 hover:text-slate-900 transition-all border border-slate-200 cursor-pointer"
          ...
          {/* Items List */}
          <div className="flex-grow overflow-y-auto px-6 py-5 space-y-3.5 custom-scrollbar bg-white">
            {cartItems.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-center py-20">
                ...
                <p className="text-slate-500 text-xs max-w-xs leading-relaxed font-normal">
                  {isAr
              ...
              <>
                {cartItems.map((item) => (
                  <div
                    key={item.id}
                    className="flex items-center gap-3.5 p-3.5 rounded-xl bg-slate-50 border border-slate-200 hover:border-blue-400 transition-all duration-200 group"
                  >
                    {/* صورة مصغرة */}
                    <div className="w-14 h-14 rounded-lg bg-white overflow-hidden flex-shrink-0 border border-slate-200 relative">
                      <img
                        src={item.image}
                        alt={item.name}
                    ...
                    <div className="flex-grow min-w-0 z-10">
                      ...
                      <p className="text-slate-500 text-[11px] truncate mt-0.5 font-normal">
                      ...
                      {item.price ? (
                        <div className="mt-1.5 flex flex-wrap items-baseline gap-2">
                          <span className="text-blue-700 text-xs font-bold">
                            {item.price.toFixed(2)} <span className="text-[10px] text-slate-500">{isAr ? 'د.ل' : 'LYD'}</span>
                          </span>
                          {item.quantity > 1 && (
                            <span className="text-slate-500 text-[10px]">
                              ({isAr ? 'المجموع:' : 'Total:'} <span className="text-slate-900 font-semibold">{(item.price * item.quantity).toFixed(2)} {isAr ? 'د.ل' : 'LYD'}</span>)
                            </span>
                          )}
                        </div>
                      ) : (
                        <span className="text-slate-600 text-[10px] font-medium mt-1 bg-slate-200/70 border border-slate-300 px-2 py-0.5 rounded inline-block">
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        <div className="flex items-center bg-white border border-slate-300 rounded-lg p-0.5 shadow-2xs">
                          <button
                            ...
                            className="p-1 text-slate-600 hover:text-slate-900 hover:bg-slate-100 rounded active:scale-90 transition-all cursor-pointer"
                          >
                ...
                <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-3 text-right mt-4">
                  ...
                  <div className="space-y-2 text-xs">
                    ...
                    <div className="p-2.5 rounded-lg bg-white border border-slate-200 space-y-1">
                      <div className="flex justify-between items-center text-[11px]">
                        ...
                        <span className="text-slate-500 text-[10px]">{isAr ? 'معرض الانارة الحديثة-همالي قرقوم' : 'Modern Enarah Showroom'}</span>
                      </div>
                      <div className="flex justify-between items-center bg-slate-50 px-2.5 py-1.5 rounded border border-slate-200">
                        <span className="font-mono font-bold text-slate-900 tracking-wider text-xs">115007000090012</span>
                        <button
                          onClick={() => handleCopy('115007000090012')}
                          className="px-2 py-0.5 rounded text-slate-700 hover:text-slate-900 bg-slate-200 hover:bg-slate-300 transition-all text-[10px] font-semibold flex items-center gap-1 cursor-pointer"
                        >
                    ...
                    <div className="p-2.5 rounded-lg bg-white border border-slate-200 space-y-1">
                      <div className="flex justify-between items-center text-[11px]">
                        ...
                        <span className="text-slate-500 text-[10px]">{isAr ? 'مصطفي محمد سليمان قرقوم' : 'Mustafa Mohamed Qarqoum'}</span>
                      </div>
                      <div className="flex justify-between items-center bg-slate-50 px-2.5 py-1.5 rounded border border-slate-200">
                        <span className="font-mono font-bold text-slate-900 tracking-wider text-xs">0014264755001</span>
                        <button
                  ...
                  <div className="pt-2 border-t border-slate-200 text-[11px] text-slate-600 flex items-center gap-2 leading-relaxed font-normal">
          ...
          {/* Checkout Footer */}
          {cartItems.length > 0 && (
            <div className="p-5 border-t border-slate-200 bg-slate-50 space-y-3.5 relative z-20">
              <div className="flex items-center justify-between text-xs font-semibold">
                <span className="text-slate-600">{isAr ? 'عدد الأصناف المطلوبة:' : 'Total items:'}</span>
                <span className="text-slate-900 font-bold bg-white border border-slate-200 px-2.5 py-0.5 rounded shadow-2xs">
                  {cartCount}
                </span>
              </div>
              ...
              {/* Trust Advice Banner */}
              <div className="p-2.5 rounded-lg bg-white border border-slate-200 text-slate-600 text-xs text-center leading-relaxed font-normal flex items-center justify-center gap-1.5 shadow-2xs">
              ...
              <button
                ...
                className="w-full py-3.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-bold text-sm flex items-center justify-center gap-2 cursor-pointer transition-all duration-200 active:scale-95 shadow-md shadow-emerald-500/20"
              >
              ...
              <button
                onClick={clearCart}
                className="w-full py-2 text-slate-500 hover:text-rose-600 rounded-lg text-xs transition-colors cursor-pointer"
              >
